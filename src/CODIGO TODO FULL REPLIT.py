...
class Task(ft.Column):
    ...
    def login_clicked(self, e):
        ...
        url = f"{OAUTH_URL}?client_id={GITHUB_CLIENT_ID}&redirect_uri=https://to-do-app--dany155.replit.app/oauth_callback&scope=read:user&state={state}"
        self.on_login(url)
...
class TodoApp(ft.Column):
    ...
    def load_tasks(self):
        ...
        try:
            ...
            if os.path.exists(PARQUET_FILE):
                result = duckdb.sql(f"SELECT task_data FROM '{PARQUET_FILE}' WHERE user_id = '{self.user_id}'").fetchone()
        ...
        if not tasks_data:
            try:
                if hasattr(self._page_ref, 'client_storage') and SECRET_KEY:
                    ...
                    stored_encrypted = self._page_ref.client_storage.get(storage_key)
                    if stored_encrypted:
    ...
    def save_tasks(self):
        ...
        for task in self.tasks.controls:
            tasks_list.append({"task_id": task.task_id, "task_name": task.task_name, "completed": task.completed})
        ...
        try:
            ...
            if os.path.exists(PARQUET_FILE):
                try:
                    all_rows = duckdb.sql(f"SELECT user_id, task_data FROM '{PARQUET_FILE}' WHERE user_id != '{self.user_id}'").fetchall()
                    for row in all_rows:
            ...
            if all_data:
                con = duckdb.connect()
                con.execute("CREATE TEMPORARY TABLE dados(user_id VARCHAR, task_data VARCHAR)")
                for uid, tdata in all_data:
                    con.execute("INSERT INTO dados VALUES (?, ?)", [uid, tdata])
    ...
    def add_task_to_ui(self, task_name, task_id=None, completed=False):
        task = Task(task_name=task_name, task_id=task_id, completed=completed, on_status_change=self.task_status_change, on_delete=self.task_delete)
...
async def main(page: ft.Page):
    ...
    provider = GitHubOAuthProvider(
        ...
        redirect_url="https://to-do-app--dany155.replit.app/oauth_callback",
    )
